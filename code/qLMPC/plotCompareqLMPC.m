function y = plotCompareqLMPC(x1,x2,x4,tspan,dt)
% x1 = qLMPC- Anticipatory
% x2 = qLMPC-Pablo
% x3 = KMPC2
% x4 = Ref
clear ph
...
% Plot KMPC results
for i = 1:2
    ph(2+i) = plot(tspan,x2(:,i),'-','Color',ccolors_valid(i+2,:),'LineWidth',4);
end
% % Plot KMPC results
% for i = 1:2
...
title('Comparison of qLPMC with current and anticipatory reference', 'FontSize', 30, 'FontWeight', 'bold')
xlabel('\bf{Time (s)}','FontSize',30)
ylabel('\bf{\phi_i} (degree)','FontSize',30)
grid on
legend([ph(1) ph(3) ph(7)],{' \bf{qLMPC}_1',' \bf{qLMPC}_2',' \bf{Ref}'})
